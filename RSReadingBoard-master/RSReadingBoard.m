...
@property (nonatomic, weak) IBOutlet NSLayoutConstraint *lclTitleTop; //title与iamgeview的垂直边距

@property (nonatomic, weak) IBOutlet NSLayoutConstraint *lclTitleTrailing; //title与iamgheview的右对齐边距 -20

@property (nonatomic, weak) IBOutlet NSLayoutConstraint *lcvColorLeading;//疏港与scroll的水平间距

@property (nonatomic, weak) IBOutlet NSLayoutConstraint *lcivImageTrailing;//scrollView和ImageView的水平间距

@property (nonatomic, weak) IBOutlet NSLayoutConstraint *lcivImageTop;//imageview与scroll的顶部垂直间距

@property (nonatomic, weak) IBOutlet NSLayoutConstraint *lcivImageBottom; //scroll与imageview的底部垂直间距
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        ...
        self.vColor.backgroundColor=[UIColor purpleColor];
...
- (void)viewDidLoad
{
    ...
    //保存各种原始属性值
    [_oldConstants setObject:@(self.lcvColorLeading.constant) forKey:PROPERTY_NAME(self.lcvColorLeading)];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (article && article.body) {
            ...
            while (lastGlyph < self.layoutManager.numberOfGlyphs) {
                ...
                if (y == CGFLOAT_MAX) { //iphone竖屏的时候，并且图片数目小于页数
                    for (RSClipView *clipView in self.clipViews) {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if (scrollView == self.vContent) {
        ...
        if (currentPage == 0) {
            ...
            if ([[UIApplication sharedApplication] statusBarOrientation] != UIInterfaceOrientationPortrait) {//不是竖屏
            ...
            if ([[UIApplication sharedApplication] statusBarOrientation] != UIInterfaceOrientationPortrait) {//不是竖屏
                ...
                //往上移动
                self.lclTitleTop.constant = -self.lcivImageHeight.constant
                + roundf((self.contentInsets.top - self.lTitle.bounds.size.height) / 2.0f);
